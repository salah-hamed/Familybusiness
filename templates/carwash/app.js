...
function formatDate(value){ const d=toDate(value); return d && !Number.isNaN(d.getTime()) ? d.toLocaleDateString("ar-EG") : "-"; }

function readProfile(){ try { return JSON.parse(localStorage.getItem(storageKey()) || "null"); } catch { return null; } }
function collectProfile(){ return { customerName:$("customerName").value.trim(), customerPhone:$("customerPhone").value.trim(), carModel:$("carModel").value.trim(), carColor:$("carColor").value.trim(), plateNumber:$("plateNumber").value.trim(), customerAddress:$("customerAddress").value.trim(), location:$("location").value, firstWashDate:$("firstWashDate").value, preferredTime:$("preferredTime").value, notes:$("notes").value.trim() }; }
function saveProfile(profile){ localStorage.setItem(storageKey(),JSON.stringify(profile)); savedProfile=profile; }
function fillProfile(profile){ if(!profile)return; ["customerName","customerPhone","carModel","carColor","plateNumber","customerAddress","location","firstWashDate","preferredTime","notes"].forEach(id=>{ if($(id)&&profile[id])$(id).value=profile[id]; }); $("savedNotice")?.classList.remove("hidden"); }
function validateProfile(profile){ if(!profile.customerName||!profile.customerPhone||!profile.carModel||!profile.carColor||!profile.plateNumber||!profile.customerAddress||!profile.firstWashDate||!profile.preferredTime)return "من فضلك أكمل بيانات العميل والسيارة والعنوان وأول موعد للغسيل."; if(!/^01\d{9}$/.test(profile.customerPhone.replace(/\s/g,"")))return "من فضلك أدخل رقم موبايل مصري صحيح مكوّن من 11 رقمًا."; if(new Date(`${profile.firstWashDate}T23:59:59`) < new Date())return "من فضلك اختر موعدًا من اليوم أو بعده."; return null; }

function isRenewable(sub){
  if(!sub)return true;
  const expires=toDate(sub.expiresAt);
  return sub.status!=="active" || Number(sub.remainingWashes||0)<=0 || (expires && expires<new Date());
}
...
async function loadWashHistory(key){
  ...
  if(!box||!key)return;
  try{
    const snap=await getDocs(query(collection(db,"carwashSubscriptions",key,"washes"),orderBy("completedAt","desc")));
    box.innerHTML="";
    snap.forEach(ds=>{
      const wash=ds.data();
    ...
    if(snap.empty)box.innerHTML='<div class="helperText">لا توجد غسلات منفذة بعد.</div>';
  }catch{ box.innerHTML='<div class="helperText">تعذر تحميل سجل الغسلات الآن.</div>'; }
}
...
async function showSubscription(sub){
  ...
  $("subscriptionProgress").style.width=total>0?`${Math.min(100,Math.max(0,(used/total)*100))}%`:"0%";
  const expires=toDate(sub.expiresAt), expired=expires ? expires<new Date() : false, renewable=isRenewable(sub);
  ...
  $("nextWashText").innerText=!renewable&&next ? `${next.toLocaleDateString("ar-EG")} — ${sub.preferredTime||"الوقت المتفق عليه"}` : renewable ? "لا توجد غسلة قادمة حتى تجديد الاشتراك." : "سيتم تحديد الموعد بعد التفعيل.";
  const daysLeft=expires?Math.ceil((expires-new Date())/86400000):null;
  const warning=$("renewWarning");
  if(!renewable&&daysLeft!==null&&daysLeft>=0&&daysLeft<=3){ warning.innerText=`🔔 اشتراكك ينتهي خلال ${daysLeft===0?"اليوم":`${daysLeft} يوم`}. جهّز التجديد للشهر الجديد.`; warning.classList.add("show"); } else warning.classList.remove("show");
...
async function loadSavedSubscription(){
  ...
  try{return await loadSubscriptionByIdentity(savedProfile.customerPhone,savedProfile.plateNumber);}catch{return false;}
}

async function sendSubscriptionRequest(isRenewal=false){
  const profile=collectProfile(), error=validateProfile(profile), statusBox=isRenewal?$("renewStatus"):$("status");
  if(error){statusBox.innerText=error;return;}
  if(currentSubscription&&!isRenewable(currentSubscription)){statusBox.innerText="اشتراكك الحالي ما زال نشطًا ولا يحتاج تجديد الآن.";return;}
  ...
  try{
    ...
    const requestState=await getDoc(doc(db,"carwashSubscriptionRequests",key));
    if(requestState.exists()&&requestState.data().status==="pending"){showPendingState();statusBox.innerText="لديك طلب قيد المراجعة بالفعل.";btn.disabled=false;return;}
    const order={ projectId:currentProjectId, providerId:currentProjectId, templateType:"carwash", serviceType:"external_car_wash", planType:"monthly_new", renewal:isRenewal, subscriptionKey:key, packageWashes:pricing.monthlyWashes, ...profile, visitDate:profile.firstWashDate, visitTime:profile.preferredTime, price:pricing.monthly, status:"new" };
    const res=await createOrder(order,{requestKey:key,renewal:isRenewal});
    if(res.success){ saveProfile(profile); currentSubscriptionKey=key; statusBox.innerText=isRenewal?"تم إرسال طلب تجديد الاشتراك بنجاح 🎉":"تم إرسال طلب الاشتراك الشهري بنجاح 🎉"; showPendingState(); }
...
$("refreshSubscriptionBtn").onclick=async()=>{ $("subscriptionMessage").innerText="جاري تحديث الحالة..."; const profile=readProfile(); const ok=profile?.customerPhone&&profile?.plateNumber ? await loadSubscriptionByIdentity(profile.customerPhone,profile.plateNumber) : false; if(!ok)$("subscriptionMessage").innerText="لم يتم تفعيل اشتراكك بعد. إذا أرسلت الطلب حديثًا انتظر تأكيد مقدم الخدمة."; };
$("lookupSubscriptionBtn").onclick=async()=>{const phone=$("lookupPhone").value.trim(),plate=$("lookupPlate").value.trim();if(!phone||!plate){$("lookupStatus").innerText="اكتب رقم الهاتف ورقم اللوحة.";return;}$("lookupStatus").innerText="جاري البحث...";try{const ok=await loadSubscriptionByIdentity(phone,plate,{saveIdentity:true});$("lookupStatus").innerText=ok?"تم العثور على اشتراكك ✅":"لم نجد اشتراكًا بهذه البيانات.";}catch{$("lookupStatus").innerText="تعذر استرجاع الاشتراك الآن.";}};
...
async function init(){
  ...
  $("firstWashDate").min=new Date().toISOString().split("T")[0];
