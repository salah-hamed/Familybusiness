import { dashboardTemplates, templateFromProjectLink } from "./template-config.js";
...
function getProjectLink() { return projectLinkInput?.value?.trim() || ""; }
function getBusinessName() {
  return businessNameInput?.value?.trim() || document.getElementById("userName")?.innerText?.trim() || "مشروعي";
}
function getTemplate() { return templateFromProjectLink(getProjectLink()); }
...
function buildMarketingMessage(tone) {
  ...
  if (template === "carwash") {
    const messages = {
      short: `🚗 ${name}\nغسيل خارجي لسيارتك في مكانها وبموعد يناسبك.\n${link}`,
      offer: `✨ خلي عربيتك أنضف من غير ما تتحرك من مكانك.\nمع ${name} احجز غسيل السيارة الخارجي وحدد المكان والموعد بسهولة.\nاحجز من هنا 👇\n${link}`,
      personal: `أهلاً 👋\nلو محتاج غسيل خارجي لعربيتك، ${name} يوصلك لمكان السيارة في الموعد اللي تختاره.\nده رابط الحجز:\n${link}`
    };
...
async function copyText(text, successMessage) {
  if (!text) return;
  try { await navigator.clipboard.writeText(text); marketingStatus.innerText = successMessage; }
  catch { marketingStatus.innerText = "تعذر النسخ تلقائيًا — يمكنك تحديد النص ونسخه يدويًا"; }
}
...
document.getElementById("copyMarketingTextBtn")?.addEventListener("click", async () => copyText(marketingMessage.value, "تم نسخ الرسالة التسويقية ✅"));
document.getElementById("copyMarketingLinkBtn")?.addEventListener("click", async () => copyText(getProjectLink(), "تم نسخ رابط المشروع ✅"));
...
document.getElementById("nativeMarketingShareBtn")?.addEventListener("click", async () => {
  ...
  const text = marketingMessage.value || buildMarketingMessage(marketingTone?.value || "short");
  if (navigator.share) {
    try { await navigator.share({ title: getBusinessName(), text, url: link }); return; }
...
const projectReadyTimer = setInterval(() => {
  attempts++;
  if (getProjectLink() || attempts >= 20) { clearInterval(projectReadyTimer); refreshMessage(); }
}, 500);
