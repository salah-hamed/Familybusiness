...
onAuthStateChanged(auth, async (user) => {
  ...
  try {
    ...
    if (!data.isActive) {
      ...
      document.getElementById("copyLinkBtn").style.display = "none";
      status.innerHTML = `
        <div style="background:#fff3cd;color:#856404;padding:15px;border-radius:10px;margin:15px 0;border:1px solid #ffeeba;text-align:center;font-weight:bold;">
    ...
    userEmail.innerText = "";
    projectType.innerText = "Project: " + (data.projectId || data.template || "cleaning");
    status.innerText = "Active: " + data.isActive;

    const projectSlug = data.template;
    projectLink.value =
      `${window.location.origin}/Familybusiness/templates/${projectSlug}/?project=${encodeURIComponent(currentProjectId)}`;
...
async function loadOrders(projectId) {
  ...
  try {
    ...
    document.getElementById("acceptedOrders").innerText = 0;
    document.getElementById("doneOrders").innerText = 0;
    document.getElementById("canceledOrders").innerText = 0;
    ...
    snapshot.forEach((docSnap) => {
      ...
      card.innerHTML = `
<div style="background:#ffffff;border-radius:18px;padding:18px;box-shadow:0 8px 20px rgba(0,0,0,0.08);margin-bottom:15px;font-family:Arial;">
  <div style="display:flex;justify-content:space-between;align-items:center;">
    <h3 style="margin:0;color:#111;">${order.customerName || "عميل"}</h3>
    <span style="padding:6px 12px;border-radius:20px;color:#fff;font-size:12px;background:${
      order.status === "new" ? "#f59e0b" :
      order.status === "accepted" ? "#3b82f6" :
      order.status === "done" ? "#22c55e" : "#ef4444"
  ...
  <hr style="margin:12px 0;">
  <p style="font-size:18px;font-weight:bold;color:#4f46e5;">💰 ${order.price || 0} جنيه</p>
  <a href="https://wa.me/${(order.customerPhone || "").replace(/\D/g,'')}" target="_blank" style="display:block;background:#25D366;color:#fff;text-align:center;padding:12px;border-radius:12px;text-decoration:none;margin:10px 0;font-weight:bold;">💬 تواصل واتساب</a>
  ${order.status === "new" ? `
    <div style="display:flex;gap:8px;flex-wrap:wrap;">
      <button class="acceptBtn" style="flex:1;padding:10px;">قبول</button>
      <button class="cancelBtn" style="flex:1;padding:10px;">إلغاء</button>
    </div>` : order.status === "accepted" ? `
    <div style="display:flex;gap:8px;flex-wrap:wrap;">
      <button class="doneBtn" style="flex:1;padding:10px;">تم التنفيذ</button>
      <button class="cancelBtn" style="flex:1;padding:10px;">إلغاء</button>
    </div>` : ""}
</div>`;
  ...
  buttons.forEach((btn) => {
    btn.onclick = () => {
      ...
      Array.from(cards).forEach((card) => {
        ...
        card.style.display = selectedTab === "all" || cardStatus === selectedTab
          ? "block"
          : "none";
      });
    };
